...
Every read is behind the account JWT (spec §122): health data answers only
the owner's web session, never the bare URL. The web never fetches raw
firehoses: raw reads are capped and viewport aggregation goes through
TimescaleDB ``time_bucket`` so the database, not the browser, does the
folding.
"""
